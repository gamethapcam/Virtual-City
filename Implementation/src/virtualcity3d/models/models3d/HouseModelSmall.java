package virtualcity3d.models.models3d;

import framework.core.models3d.Model3DBase;
import resources.AssetManager;
import resources.Assets3D;

/**
 * Created with IntelliJ IDEA.
 * User: Yan
 * Date: 25/09/13
 * Time: 11:35
 * To change this template use File | Settings | File Templates.
 */
public class HouseModelSmall extends Model3DBase {

    //approximated size
    private static final int X_SIZE = 10;
    private static final int Y_SIZE = 11;
    private static final int Z_SIZE = 10;

    public HouseModelSmall() {
        super(AssetManager.getAsset3D(Assets3D.SMALL_HOUSE));
    }

    @Override
    public double getX_Size() {
        return X_SIZE;
    }

    @Override
    public double getY_Size() {
        return Y_SIZE;
    }

    @Override
    public double getZ_Size() {
        return Z_SIZE;
    }
}
